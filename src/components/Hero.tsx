export default function Home() {
    return (
      ...
      Crafting 
          <span className="relative inline-block mx-2 bg-orange-400 transform -rotate-6 px-2 py-1 rounded-2xl hover:rotate-0 transition-transform duration-500 ease-in-out">
           ideas
            {/* Decorative icon pinned to the edge */}
            <span className="absolute top-0 [right:-0.5rem] -translate-x-1/4 -translate-y-1/4 text-black text-3xl [rotate:120deg]">➤</span>
          </span>{" "}
    ...
    <div className="w-48 h-48 md:w-64 md:h-64 flex-shrink-0 relative group">
      <img
        src="/ronnel-alt.webp"
        alt="Your Name"
        className="w-full h-full object-cover rounded-full shadow-lg transition-opacity duration-500 ease-in-out group-hover:opacity-0"
      />
      <img
        src="/ronnel.webp" // new photo on hover
        alt="Your Name alternate"
        className="w-full h-full object-cover rounded-full shadow-lg absolute top-0 left-0 opacity-0 transition-opacity duration-500 ease-in-out group-hover:opacity-100"
      />
    </div>
  
    {/* Right: Intro */}
    <div className="flex-1 text-center md:text-left">
      <h1 className="text-4xl font-bold mb-4">Hi, I’m Ronnel</h1>
